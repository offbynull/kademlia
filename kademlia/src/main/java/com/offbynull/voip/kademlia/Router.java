...
public final class Router {
    ...
    public void touch(Instant time, Node node) throws LinkConflictException {
        ...
        if (!bucketChangeSet.viewAdded().isEmpty()) {
            ...
            NodeChangeSet dataChangeSet = dataSet.put(node, InternalNodeProperties.IN_ROUTINGTREE, UNUSED_VALUE);
            ...
            Validate.validState(dataSet.get(node, InternalNodeProperties.IN_ROUTINGTREE) == UNUSED_VALUE);
        } else {
        ...
        if (!nearChangeSet.viewAdded().isEmpty()) {
            ...
            ActivityChangeSet activityChangeSet = activitySet.touch(time, node);
            dataSet.put(node, InternalNodeProperties.IN_NEARSET, UNUSED_VALUE);
            ...
            if (!nearChangeSet.viewRemoved().isEmpty()) {
                ...
                NodeChangeSet removeDataChangeSet = dataSet.remove(removedNode, InternalNodeProperties.IN_NEARSET);
                ...
                if (dataSet.get(removedNode, InternalNodeProperties.IN_ROUTINGTREE) == null) {
            ...
            sanityCheckChangeSet(activityChangeSet, 0, 1, 0);
            Validate.validState(dataSet.get(node, InternalNodeProperties.IN_NEARSET) != null);
        } else {
    ...
    public List<Node> getClosestNodes(Id id, int max) {
